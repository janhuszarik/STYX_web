<style>
	/* Váš CSS je v poriadku, ponechávam ho */
	#ftpImagePreview img { max-width: 200px; max-height: 200px; object-fit: contain; }
	#ftp-browser-container { max-height: 65vh; overflow-y: auto; }
	.modal-dialog { max-width: 90vw; }
	#ftp-items-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 10px; }
	.ftp-item { padding: 10px; cursor: pointer; border: 1px solid #dee2e6; border-radius: 4px; display: flex; align-items: center; gap: 12px; background-color: #fff; transition: background-color 0.2s; }
	.ftp-item:hover { background-color: #f8f9fa; }
	.ftp-item .item-icon { width: 60px; height: 60px; text-align: center; }
	.ftp-item .item-icon i { font-size: 36px; line-height: 60px; }
	...
	.ftp-breadcrumb-bar { font-size: 14px; margin-bottom: 10px; }
...
<div id="ftp-browser-container">
	<div class="mb-2 d-flex align-items-center p-2 bg-light border-bottom sticky-top">
		...
		<div id="ftp-breadcrumb-bar" class="d-inline-flex align-items-center flex-grow-1 text-truncate">
			<strong class="ms-2 me-1">Pfad:</strong>
			<span id="ftp-breadcrumb" class="text-truncate"></span>
		</div>
	</div>
	...
	document.addEventListener("DOMContentLoaded", () => {
		...
		const loadFolder = async (path) => {
			...
			try {
				...
				itemsList.innerHTML = `<div class="alert alert-danger m-2"><strong>Chyba:</strong> ${err.message}</div>`;
		...
		const renderItems = (items) => {
			...
			const html = items.map(item => {
				...
				const icon = item.type === 'dir'
					...
					: (isImage ? `<img src="${item.url}" alt="${item.name}" loading="lazy">` : '<i class="fas fa-file-alt text-secondary"></i>');
				const actionClass = item.type === 'dir' ? 'is-folder' : (isImage ? 'is-image' : '');
				const size = item.size ? `${(item.size / 1024).toFixed(2)} KB` : '';
				return `<div class="ftp-item ${actionClass}" data-path="${item.path}" data-url="${item.url || ''}">
		...
		// ✅ ROBUSTNEJŠÍ SPÔSOB SPUSTENIA
		// Tento kód počúva na kliknutie kdekoľvek na stránke.
		document.body.addEventListener('click', function(e) {
			// Zistí, či sa kliklo na tlačidlo s triedou .ftp-picker
			const pickerButton = e.target.closest('.ftp-picker');
			if (!pickerButton) return; // Ak nie, nerobí nič

			e.preventDefault();

			// Uloží si, pre ktoré polia sa obrázok vyberá
			lastTarget = pickerButton.dataset.ftpTarget;
			lastPreview = pickerButton.dataset.previewTarget;

			if (!lastTarget || !lastPreview) {
				console.error('FTP Picker button is missing data-ftp-target or data-preview-target attributes.');
				alert('Chyba konfigurácie: FTP tlačidlu chýbajú potrebné dáta.');
				return;
			}

			// Zobrazí modal a načíta obsah
