...
(function(admin, $) {
	...
	PluginSlider.prototype = {
		...
		setVars: function() {
			var $output = $( this.$el.data('plugin-sliders-output') );
			...
			if( this.$output ) {
				this.$output.on('change', function(){
					if( self.options.half_values ) {
						...
						$( $(this).parent().data('plugin-sliders-link-values-to') ).find('.min').text( values[0] );
						$( $(this).parent().data('plugin-sliders-link-values-to') ).find('.max').text( values[1] );
					} else {
						$( $(this).parent().data('plugin-sliders-link-values-to') ).find('.min').text( $(this).val() );
					}
				});
			}
...
(function($) {
	...
	if ( $.isFunction($.fn[ 'slider' ]) ) {
		...
		$(function() {
			$('[data-plugin-sliders]').each(function() {
				var $this = $( this ),
					opts = {};
